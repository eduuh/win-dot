...
#
# Layout:
#   window 1: notes     (shell in ~/projects/branch-notes-work)
#   window 2: personal  (shell in ~/projects/personal-notes)
...
function New-NotesWindow {
    ...
    if (Test-Path $Dir) {
        & $psmux @argsBase -n $WinName -c $Dir
    } else {
